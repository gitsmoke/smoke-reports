...
ok 86 - key sanity-checking
ok 87 - git -c works with aliases of builtins
ok 88 - git -c does not split values on equals
ok 89 - git -c dies on bogus config
ok 90 - git -c complains about empty key
ok 91 - git -c complains about empty key and value
