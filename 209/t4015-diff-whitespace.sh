...
ok 38 - check tabs as indentation (tab-in-indent: off)
ok 39 - check tabs as indentation (tab-in-indent: on)
ok 40 - check tabs and spaces as indentation (tab-in-indent: on)
ok 41 - check tab-in-indent and indent-with-non-tab conflict
ok 42 - check tab-in-indent excluded from wildcard whitespace attribute
ok 43 - line numbers in --check output are correct
ok 44 - checkdiff detects new trailing blank lines (1)
ok 45 - checkdiff detects new trailing blank lines (2)
ok 46 - checkdiff allows new blank lines
ok 47 - whitespace-only changes not reported
ok 48 - whitespace-only changes reported across renames
ok 49 - rename empty
ok 50 - combined diff with autocrlf conversion
ok 51 - setup diff colors
ok 52 - diff that introduces a line with only tabs
