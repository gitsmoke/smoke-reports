...
ok 71 - git log -SF -p master
ok 72 - git log --decorate --all
ok 73 - git log --decorate=full --all
ok 74 - git rev-list --parents HEAD
ok 75 - git rev-list --children HEAD
ok 76 - git whatchanged master
ok 77 - git whatchanged -p master
ok 78 - git whatchanged --root master
ok 79 - git whatchanged --root -p master
ok 80 - git whatchanged --patch-with-stat master
ok 81 - git whatchanged --root --patch-with-stat master
ok 82 - git whatchanged --root --patch-with-stat --summary master
ok 83 - git whatchanged --root -c --patch-with-stat --summary master
ok 84 - git whatchanged --root --cc --patch-with-stat --summary master
ok 85 - git whatchanged -SF master
ok 86 - git whatchanged -SF -p master
ok 87 - git log --patch-with-stat master -- dir/
ok 88 - git whatchanged --patch-with-stat master -- dir/
ok 89 - git log --patch-with-stat --summary master -- dir/
ok 90 - git whatchanged --patch-with-stat --summary master -- dir/
ok 91 - git show initial
ok 92 - git show --root initial
ok 93 - git show side
ok 94 - git show master
ok 95 - git show -c master
ok 96 - git show -m master
ok 97 - git show --first-parent master
ok 98 - git show --stat side
ok 99 - git show --stat --summary side
ok 100 - git show --patch-with-stat side
ok 101 - git show --patch-with-raw side
ok 102 - git show --patch-with-stat --summary side
ok 103 - git format-patch --stdout initial..side
ok 104 - git format-patch --stdout initial..master^
ok 105 - git format-patch --stdout initial..master
ok 106 - git format-patch --stdout --no-numbered initial..master
ok 107 - git format-patch --stdout --numbered initial..master
ok 108 - git format-patch --attach --stdout initial..side
ok 109 - git format-patch --attach --stdout --suffix=.diff initial..side
ok 110 - git format-patch --attach --stdout initial..master^
ok 111 - git format-patch --attach --stdout initial..master
ok 112 - git format-patch --inline --stdout initial..side
ok 113 - git format-patch --inline --stdout initial..master^
ok 114 - git format-patch --inline --stdout --numbered-files initial..master
ok 115 - git format-patch --inline --stdout initial..master
ok 116 - git format-patch --inline --stdout --subject-prefix=TESTCASE initial..master
ok 117 - git config format.subjectprefix DIFFERENT_PREFIX
ok 118 - git format-patch --inline --stdout initial..master^^
ok 119 - git format-patch --stdout --cover-letter -n initial..master^
ok 120 - git diff --abbrev initial..side
ok 121 - git diff -r initial..side
ok 122 - git diff --stat initial..side
ok 123 - git diff -r --stat initial..side
ok 124 - git diff initial..side
ok 125 - git diff --patch-with-stat initial..side
ok 126 - git diff --patch-with-raw initial..side
ok 127 - git diff --patch-with-stat -r initial..side
ok 128 - git diff --patch-with-raw -r initial..side
ok 129 - git diff --name-status dir2 dir
ok 130 - git diff --no-index --name-status dir2 dir
ok 131 - git diff --no-index --name-status -- dir2 dir
ok 132 - git diff --no-index dir dir3
ok 133 - git diff master master^ side
ok 134 - git diff --dirstat master~1 master~2
