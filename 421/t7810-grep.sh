...
ok 52 - log with multiple --author uses union
...
ok 53 - log with --grep and multiple --author uses all-match
...
ok 54 - log with --grep and multiple --author uses all-match
...
../x:x x xx x
ok 59 - grep from a subdirectory to search wider area (1)
expecting success: 
	mkdir -p s &&
	(
		cd s || exit 1
...
ok 60 - grep from a subdirectory to search wider area (2)
expecting success: 
...
ok 62 - outside of git repository
expecting success: 
	rm -fr is &&
	mkdir -p is/git/sub &&
...
ok 63 - inside git repository but with --no-index
...
ok 66 - grep -- pattern -- pathspec

expecting success: 
	git grep -e "->" -- double-dash >actual &&
	test_cmp expected actual

ok 67 - grep -e pattern -- path
expecting success: 
	git grep -e -- -- double-dash >actual &&
	test_cmp expected actual
